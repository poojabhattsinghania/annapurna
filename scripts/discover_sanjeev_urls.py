#!/usr/bin/env python3
"""
Discover recipe URLs from Sanjeev Kapoor's websites

Sanjeev Kapoor is a celebrity Indian chef with multiple recipe websites:
- sanjeevkapoor.com - Main site with ~8,000 recipes
- Good quality Schema.org data
"""
...
SITEMAP_URL = "https://www.sanjeevkapoor.com/sitemap.xml"
    ...
    try:
        ...
        for url_element in root.findall('.//ns:url', namespaces):
            ...
            if loc is not None:
                ...
                if '/recipe/' in url or '/recipes/' in url or 'sanjeevkapoor.com/recipe' in url:
                    urls.append(url)
        ...
        try:
            ...
            for link in soup.find_all('a', href=True):
                ...
                if '/recipe/' in href or '/recipes/' in href:
                    ...
                    if 'sanjeevkapoor.com' in full_url:
            ...
            for pattern_url in patterns:
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        if found_recipes:
                            print(f"      ✓ Found recipes (Total: {len(all_urls)})")
                            time.sleep(0.5)
...
def main():
    print("🚀 Sanjeev Kapoor Recipe URL Discovery")
    print("=" * 70)
    ...
    if sitemap_xml:
        ...
        print(f"   Found {len(sitemap_urls)} recipe sitemaps")
    ...
    if len(all_recipe_urls) > 0:
        # Save to file
        filename = 'sanjeevkapoor_urls.txt'
        save_urls_to_file(sorted(all_recipe_urls), filename)

        print(f"\n💡 Next steps:")
        print(f"   1. Review the URLs in {filename}")
        print(f"   2. Run: python scrape_sanjeev_recipes.py")
        print(f"   3. Expected scraping time: ~{len(all_recipe_urls) / 1200:.1f} hours")
    else:
        print("\n❌ No URLs discovered. Website might require:")
        print("   - JavaScript rendering (consider Selenium/Playwright)")
        print("   - Authentication or API access")
        print("   - Manual URL extraction")
