#!/usr/bin/env python3
"""
Add processing tracking fields to raw_scraped_content table (fast version)
"""
...
def add_processing_tracking_fields():
    ...
    with engine.connect() as conn:
        ...
        # Add processing_attempts if it doesn't exist (nullable first, then set default)
        if 'processing_attempts' not in existing_columns:
            print("Adding processing_attempts column...")
            conn.execute(text("""
                ALTER TABLE raw_scraped_content
                ADD COLUMN processing_attempts INTEGER
            """))
            conn.commit()
            print("Setting default values for processing_attempts...")
            conn.execute(text("""
